...
import domainPoliciesData from '@/data/domain-process-security-policies.json';

interface DomainSecurityPolicy {
  id: number;
  domainSecurityPolicy: string;
  byUser: string;
  lastChanged: string;
}

type SortField = keyof DomainSecurityPolicy | null;
type SortDirection = 'asc' | 'desc' | null;

const DomainProcessSecurityPoliciesPage = () => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [fromDate, setFromDate] = useState('10/3/2025');
  ...
  const policies: DomainSecurityPolicy[] = domainPoliciesData.domainSecurityPolicies;

  const handleSort = (field: keyof DomainSecurityPolicy) => {
    if (sortField === field) {
  ...
  const getSortIcon = (field: keyof DomainSecurityPolicy) => {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (policy) =>
          policy.domainSecurityPolicy.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <Stack gap="md">
        <Group justify="space-between" align="center">
          <Title order={2} c="#1971c2" style={{ textAlign: 'left' }}>
            Domain Process Security Policies Changed
          </Title>
          <Group>
        ...
        <Paper p="md" radius="md" style={{ backgroundColor: '#ffffff', boxShadow: '0 1px 3px rgba(0, 0, 0, 0.05)' }}>
          ...
          <Table striped highlightOnHover withTableBorder>
            <Table.Thead style={{ backgroundColor: '#1971c2' }}>
              <Table.Tr>
                <Table.Th
                  onClick={() => handleSort('domainSecurityPolicy')}
                  style={{ cursor: 'pointer', userSelect: 'none', color: 'white' }}
                >
                  Domain_Security_Policy {getSortIcon('domainSecurityPolicy')}
                </Table.Th>
                <Table.Th
            ...
            <Table.Tbody>
              {paginatedData.map((policy) => (
                <Table.Tr key={policy.id}>
                  <Table.Td>{policy.domainSecurityPolicy}</Table.Td>
...
export default DomainProcessSecurityPoliciesPage;
